                ...
                <li <?php echo $offer->isCurrent() ? 'class="'.$data['menu']['active'].'"' : null; ?>>
                    <a href="<?php echo $offer->getUrl(); ?>">
                        <?php if( isset( $data['icon']['enable'] ) ): ?>
                           <span class="icon-place"><?php echo ipSlot( 'image', array( 'id' => 'asd-menu-item-image-' . $data['menu']['name'] . '-' . $i++, 'pageId' => 10, 'height' => $data['icon']['height'], 'width' => $data['icon']['width'], 'default' => "http://dummyimage.com/{$data['icon']['width']}x{$data['icon']['height']}/f5f5f5/888" ) ); ?></span>
                        <?php endif; ?>
